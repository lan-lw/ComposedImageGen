...
def encode_with_pseudo_tokens(clip_model: CLIP, text: torch.Tensor, pseudo_tokens: torch.Tensor,#,pseudo_tokens_gen:torch.Tensor,
    ...
    # _, counts = torch.unique((text == 265).nonzero(as_tuple=True)[0], return_counts=True)  # 265 is the token of *
    # cum_sum = torch.cat((torch.zeros(1, device=text.device).int(), torch.cumsum(counts, dim=0)[:-1]))
    # first_tokens_indexes = (text == 265).nonzero()[cum_sum][:, 1]
    # rep_idx = torch.cat([(first_tokens_indexes + n).unsqueeze(0) for n in range(num_tokens)])
    # if pseudo_tokens_gen.shape[0] == x.shape[0]:
    #     if len(pseudo_tokens_gen.shape) == 2:
    #         # import pdb;pdb.set_trace()
    #         pseudo_tokens_gen = pseudo_tokens_gen.unsqueeze(1)
    #     x[torch.arange(x.shape[0]).repeat_interleave(num_tokens).reshape(
    #         x.shape[0], num_tokens), rep_idx.T] = pseudo_tokens_gen.to(x.dtype)
