...
from phi import Phi
from utils_feat import extract_image_features, device, extract_pseudo_tokens_with_phi
import os
import time
...
def fiq_generate_val_predictions(clip_model: CLIP, relative_val_dataset: Dataset, ref_names_list: List[str],
    ...
    for batch in tqdm(relative_val_loader):
        ...
        batch_tokens = torch.vstack([pseudo_tokens[0][ref_names_list.index(ref+tar+str(id.item()))].unsqueeze(0) for ref,tar,id in zip(reference_names,target_names,pair_id)])
        batch_tokens_gen = torch.vstack([pseudo_tokens[1][ref_names_list.index(ref+tar+str(id.item()))].unsqueeze(0) for ref,tar,id in zip(reference_names,target_names,pair_id)])
        ...
        text_features_reversed_gen = encode_with_pseudo_tokens(clip_model, tokenized_input_captions_reversed,
                                                           batch_tokens_gen)

        predicted_features = F.normalize((F.normalize(text_features) + F.normalize(text_features_reversed)) / 2)
        predicted_features_gen = F.normalize((F.normalize(text_features_gen) + F.normalize(text_features_reversed_gen)) / 2)

        predicted_features = F.normalize(F.normalize(predicted_features) * 0.9 + F.normalize(predicted_features_gen) * 0.1)
...
def fiq_val_retrieval(dataset_path: str, dress_type: str, clip_model_name: str, ref_names_list: List[str],
                      ...
                      generated_image_dir: Optional[str] = None) -> Dict[str, float]:
    """
    Compute the retrieval metrics on the FashionIQ validation set given the pseudo tokens and the reference names
    """
    # Load the model
    clip_model, _ = clip.load(clip_model_name, device=device, jit=False, download_root="./")
...
def cirr_generate_val_predictions(clip_model: CLIP, relative_val_dataset: Dataset, ref_names_list: List[str],
    ...
    relative_val_loader = DataLoader(dataset=relative_val_dataset, batch_size=1, num_workers=10,
    ...
    for batch in tqdm(relative_val_loader):
        ...
        batch_tokens = torch.vstack([pseudo_tokens[0][ref_names_list.index(ref)].unsqueeze(0) for ref in pairs_id])
        batch_tokens_gen = torch.vstack([pseudo_tokens[1][ref_names_list.index(ref)].unsqueeze(0) for ref in pairs_id])
        tokenized_input_captions = clip.tokenize(input_captions, context_length=77).to(device)
        text_features = encode_with_pseudo_tokens(clip_model, tokenized_input_captions, batch_tokens)

        text_features_gen = encode_with_pseudo_tokens(clip_model, tokenized_input_captions, batch_tokens_gen)
        predicted_features =F.normalize( F.normalize(text_features)*0.85 + F.normalize(text_features_gen)*0.15)

        predicted_features_list.append(predicted_features)
        target_names_list.extend(target_names)
        group_members_list.extend(group_members)
        reference_names_list.extend(reference_names)
        pair_id_list.append(pairs_id)
...
def circo_generate_test_dict(relative_test_dataset: CIRCODataset, clip_model: CLIP, index_features: torch.Tensor,
    ...
    predicted_features, query_ids = circo_generate_val_predictions(clip_model, relative_test_dataset,
    ...
    # import pdb;pdb.set_trace()
    queryid_to_retrieved_images = {int(query_id[0]): query_sorted_names[:50].tolist() for
...
def cirr_generate_test_dicts(relative_test_dataset: CIRRDataset, clip_model: CLIP, index_features: torch.Tensor,
    ...
    predicted_features, reference_names, pairs_id, group_members = \
        cirr_generate_val_predictions(clip_model, relative_test_dataset, ref_names_list, pseudo_tokens)
    ...
    # Generate prediction dicts
    # import pdb;pdb.set_trace()
    pairid_to_retrieved_images = {str(int(pair_id)): prediction[:50].tolist() for (pair_id, prediction) in zip(pairs_id, sorted_index_names)}
...
def circo_generate_val_predictions(clip_model: CLIP, relative_val_dataset: Dataset, ref_names_list: List[str],
    ...
    for batch in tqdm(relative_val_loader):
        ...
        batch_tokens = torch.vstack([pseudo_tokens[0][ref_names_list.index(ref)].unsqueeze(0) for ref in pairs_id])
        batch_tokens1 = torch.vstack([pseudo_tokens[1][ref_names_list.index(ref)].unsqueeze(0) for ref in pairs_id])
        tokenized_input_captions = clip.tokenize(input_captions, context_length=77).to(device)
        text_features = encode_with_pseudo_tokens(clip_model, tokenized_input_captions, batch_tokens)
        text_features1 = encode_with_pseudo_tokens(clip_model, tokenized_input_captions, batch_tokens1)


        predicted_features = F.normalize(text_features)
        # predicted_features = F.normalize(F.normalize(text_features) + F.normalize(text_features1) * 0.5)

        predicted_features_list.append(predicted_features)
        target_names_list.extend(target_names)
        gts_img_ids_list.extend(gt_img_ids)
        query_ids_list.append(pairs_id)

    predicted_features = torch.vstack(predicted_features_list)

    return predicted_features, query_ids_list#target_names_list, gts_img_ids_list
...
def main():
    ...
    parser.add_argument("--generated-image-dir", type=str, default=None,
                        help="Directory containing generated images named as 00000.png, 00001.png, ...")
    ...
    if args.eval_type == 'oti':
        ...
        clip_model, clip_preprocess = clip.load(clip_model_name, device='cpu', jit=False, download_root="./")
        ...
        if args.eval_type == 'phi':
            ...
            clip_model_name = hyperparameters['clip_model_name']
            clip_model, clip_preprocess = clip.load(clip_model_name, device=device, jit=False, download_root="./")
        ...
        if args.dataset.lower() == 'fashioniq':
            relative_val_dataset = FashionIQDataset(
                args.dataset_path, 'val', ['dress', 'shirt','toptee'], 'relative', preprocess,
                no_duplicates=True, generated_image_dir=args.generated_image_dir
            )
        elif args.dataset.lower() == 'cirr':
            relative_val_dataset = CIRRDataset(
                args.dataset_path, 'val', 'relative', preprocess, generated_image_dir=args.generated_image_dir
            )
        elif args.dataset.lower() == 'circo':
            relative_val_dataset = CIRCODataset(
                args.dataset_path, 'val', 'relative', preprocess, generated_image_dir=args.generated_image_dir
            )
        else:
        ...
        pseudo_tokens, ref_names_list = extract_pseudo_tokens_with_phi(clip_model, phi, relative_val_dataset,dataset_name=args.dataset.lower())
    ...
    if args.dataset.lower() == 'fashioniq':
        ...
        for dress_type in ['dress','shirt', 'toptee']:
            fiq_metrics = fiq_val_retrieval(
                args.dataset_path, dress_type, clip_model_name, ref_names_list,
                pseudo_tokens, preprocess, generated_image_dir=args.generated_image_dir
            )
        ...
        cirr_metrics = cirr_val_retrieval(
            args.dataset_path, clip_model_name, ref_names_list, pseudo_tokens,
            preprocess, generated_image_dir=args.generated_image_dir
        )
        ...
        circo_metrics = circo_val_retrieval(
            args.dataset_path, clip_model_name, ref_names_list, pseudo_tokens,
            preprocess, generated_image_dir=args.generated_image_dir
        )
