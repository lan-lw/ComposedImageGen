...
class FashionIQDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 generated_image_dir: Optional[Union[Path, str]] = None,
                 generated_image_dir_alt: Optional[Union[Path, str]] = None):
        """
        ...
        self.generated_image_dir = Path(generated_image_dir) if generated_image_dir is not None else None
        self.generated_image_dir_alt = Path(generated_image_dir_alt) if generated_image_dir_alt is not None else None
        ...
        seen = set()
        for dress_type in dress_types:
            with open(self.dataset_path / 'captions' / f'cap.{dress_type}.{split}.json') as f:
                data = json.load(f)
                new_data = []
                id_counter = 0
                for da in data:
                    da['pairid'] = id_counter
                    seen.add(da['candidate'])
                    new_data.append(da)
                    id_counter += 1
                self.lengths.append(len(new_data))
                self.triplets.extend(new_data)

        # get the image names
        self.image_names = []
        for dress_type in dress_types:
            with open(self.dataset_path / 'image_splits' / f'split.{dress_type}.{split}.json') as f:
                self.image_names.extend(json.load(f))

        print(f"FashionIQ {split} - {dress_types} dataset in {mode} mode initialized")

    def _make_generated_image_path(self, category: Optional[str], pair_id: int) -> Optional[Path]:
        """Build generated image path using provided directories, if available."""
        if self.generated_image_dir is None:
            return None
        # Support templates with category placeholder
        base = str(self.generated_image_dir)
        if category is not None and '{category}' in base:
            base = base.format(category=category)
        path = Path(base) / f"{pair_id:05d}.png"
        if path.exists():
            return path
        if self.generated_image_dir_alt is not None:
            base_alt = str(self.generated_image_dir_alt)
            if category is not None and '{category}' in base_alt:
                base_alt = base_alt.format(category=category)
            alt_path = Path(base_alt) / f"{pair_id:05d}.png"
            if alt_path.exists():
                return alt_path
        # Return primary even if not exists to keep behavior predictable
        return Path(base) / f"{pair_id:05d}.png"
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    ...
                    gen_path = self._make_generated_image_path(self.dress_types[0] if len(self.dress_types) >= 1 else None, pair_id)
                    generated_image = self.preprocess(PIL.Image.open(gen_path))
...
class CIRRDataset(Dataset):
   ...
   The dataset can be used in 'relative' or 'classic' mode:
    ...
    def __init__(self,
        ...
        with open(self.dataset_path / 'cirr' / 'captions' / f'cap.rc2.{split}.json') as f:
        ...
        with open(self.dataset_path / 'cirr' / 'image_splits' / f'split.rc2.{split}.json') as f:
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    ...
                    target_image = self.preprocess(PIL.Image.open(target_image_path))
                    pair_id = self.triplets[index]['pairid']
                    generated_image_path = self.generated_image_dir / f"{pair_id:05d}.png"
                    generated_image = self.preprocess(PIL.Image.open(str(generated_image_path)))
...
class CIRCODataset(Dataset):
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            generated_image_path = self.generated_image_dir / f"{int(query_id):05d}.png"
            generated_image = self.preprocess(PIL.Image.open(str(generated_image_path)))
